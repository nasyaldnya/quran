											...
											10,		35,		200,	20, m_pCollectionManager, SS_CENTER)
{
	...
	// Signal WinMain that the window is ready. WinMain is blocked on the
	// corresponding future and proceeds to construct the engine only after this.
	// If construction threw before reaching here, the tray thread's catch block
	// calls hwndPromise.set_exception() so WinMain unblocks and rethrows.
...
TrayWindow::~TrayWindow()
{
	...
	// Use exitEngine() — not abortDelay(). abortDelay() maps to Command::Next
	// which advances the wallpaper during shutdown and overwrites the saved
	// session. exitEngine() enqueues Command::Exit: a pure wake-up that does
	// not change wallpaper state.
	Delay::exitEngine();
	Delay::exiting = true; // belt-and-suspenders for code still polling this flag
}
...
LRESULT TrayWindow::HandleMessage(HWND, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case SlideshowEngine::WM_ENGINE_TICK:
		// Engine emits this ~1 Hz. Update the countdown in all visible Players.
		Player::updateTimer();
		return 0;

	case SlideshowEngine::WM_ENGINE_WALLPAPER:
		// Wallpaper changed (or stopped). Apply to desktop and redraw buttons.
		WinUtils::updateDesktopBackground();
		Player::redrawPlayers();
		return 0;

	case SlideshowEngine::WM_ENGINE_STATE_CHANGED:
		// Engine state changed (wParam = SlideshowEngine::State).
		// Redraw player buttons so active-state icons update.
		Player::redrawPlayers();
		return 0;
