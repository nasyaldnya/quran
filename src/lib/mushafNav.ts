import { SURAH_NAMES, SURAH_NAMES_AR, VERSE_COUNTS } from '@/lib/utils'

// Page number where each surah starts (Madani mushaf standard layout)
export const SURAH_PAGES: Record<number, number> = {
  1:1,2:2,3:50,4:77,5:106,6:128,7:151,8:177,9:187,10:208,
  11:221,12:235,13:249,14:255,15:262,16:267,17:282,18:293,19:305,20:312,
  21:322,22:332,23:342,24:350,25:359,26:367,27:377,28:385,29:396,30:404,
  31:411,32:415,33:418,34:428,35:434,36:440,37:446,38:453,39:458,40:467,
  41:477,42:483,43:489,44:496,45:499,46:502,47:507,48:511,49:515,50:518,
  51:520,52:523,53:526,54:528,55:531,56:534,57:537,58:542,59:545,60:549,
  61:551,62:553,63:554,64:556,65:558,66:560,67:562,68:564,69:566,70:568,
  71:570,72:572,73:574,74:575,75:577,76:578,77:580,78:582,79:583,80:585,
  81:586,82:587,83:587,84:589,85:590,86:591,87:591,88:592,89:593,90:594,
  91:595,92:595,93:596,94:596,95:597,96:597,97:598,98:598,99:599,100:599,
  101:600,102:600,103:601,104:601,105:601,106:602,107:602,108:602,109:603,
  110:603,111:603,112:604,113:604,114:604,
}

// Page number where each juz starts
export const JUZ_PAGES: number[] = [
  1,22,42,62,82,102,121,142,162,182,
  201,222,242,262,282,302,322,342,362,382,
  402,422,442,462,482,502,522,542,562,582,
]

// Hizb data (60 hizbs, 2 per juz)
export const HIZB_PAGES: number[] = [
  1,12,22,32,42,52,62,72,82,92,
  102,112,121,132,142,152,162,172,182,192,
  201,212,222,232,242,252,262,272,282,292,
  302,312,322,332,342,352,362,372,382,392,
  402,412,422,432,442,452,462,472,482,492,
  502,512,522,532,542,552,562,572,582,592,
]

export interface SurahNavItem {
  number: number
  nameAr: string
  nameEn: string
  page: number
  verses: number
}

export function getSurahList(): SurahNavItem[] {
  return Array.from({ length: 114 }, (_, i) => {
    const n = i + 1
    return {
      number: n,
      nameAr: SURAH_NAMES_AR[n] ?? '',
      nameEn: SURAH_NAMES[n] ?? '',
      page: SURAH_PAGES[n] ?? 1,
      verses: VERSE_COUNTS[n] ?? 0,
    }
  })
}
