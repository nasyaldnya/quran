...
export const useMushafStore = create<MushafState>()(
  persist(
    (set, get) => ({
      isMushafMode: false,
      currentPage: 1,
      toggleMushafMode: () => set((s) => ({ isMushafMode: !s.isMushafMode })),
      setPage: (page) => set({ currentPage: Math.max(1, Math.min(604, page)) }),
      nextPage: () => set((s) => ({ currentPage: Math.min(604, s.currentPage + 1) })),
      prevPage: () => set((s) => ({ currentPage: Math.max(1, s.currentPage - 1) })),
    }),
    { name: 'quran-mushaf', partialize: (s) => ({ currentPage: s.currentPage }) }
  )
)

// Surah-to-page mapping (page where each surah starts in the standard Madani mushaf)
